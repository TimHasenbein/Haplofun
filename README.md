...
| GWAS information (`.csv`) | Genome-Wide Association Study file for mapping trait-associated SNPs to haplogroups. Comma-separated file with the format: `chromosome, start, end, name, score, strand`, followed by `trait, risk allele, pval, effect_size`. |
| QTL information (`.csv`) | Quantitative Trait Loci file for mapping QTLs to haplogroups. Comma-separated file with the format: `chromosome, start, end, name (REF/effect allele), score, strand`, followed by `tissue, pip, effect size (allelic fold change), target gene`. |
| Annotation information (`.csv`) | BED6 annotation file for mapping genomic features to haplogroups: `chromosome, start, end, name, score, strand`. |
