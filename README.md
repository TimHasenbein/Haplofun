...
| GWAS information (`.csv`) | Genome-Wide Association Study file for mapping trait-associated SNPs to haplogroups. Comma-separated, BED-format based: `chromosome, start, end, name, score, strand`, followed by `trait, risk allele, pval, effect_size`. |
| QTL information (`.csv`) | Quantitative Trait Loci file for mapping QTLs to haplogroups. Comma-separated, BED-format based: `chromosome, start, end, name (REF/effect allele), score, strand`, followed by `tissue, pip, effect size (allelic fold change), target gene`. |
