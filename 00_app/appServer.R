...
######------  helper ------######
# resource/performance limits for the public server deployment
# increase these parameters to allow larger uploads and heavier computations
MAX_UPLOAD_SIZE_BYTES <- 1500 * 1024^2       # hard cap on uploaded file size
FILESIZE_THRESHOLD_BYTES <- 500 * 1024       # soft "this file is large" warning
MAX_VARIANTS <- 1000                         # hard cap on variants for interactive processing
MAX_SAMPLES <- 2550                          # hard cap on samples for interactive processing
COMPUTE_TIMEOUT_SECONDS <- 20                # default timeout for haplotype/table/plot computations
LD_SCAN_TIMEOUT_SECONDS <- 60                # LD r^2 matrix calculation
NETWORK_PLOT_TIMEOUT_SECONDS <- 10           # interactive network plot render
MAX_HAPLOTYPES_FOR_NETWORK <- 50             # maximum number of haplogroups
MAX_HEATMAP_CELLS <- 1000000                 # allele heatmap size gate (haplotypes x variants)
PERFORMANCE_WARNING_CELLS <- 350000          # soft warning banner threshold (variants x samples)
...
get_variant_names <- function(vcf_obj) {
  ...
  ids[is_missing] <- paste0(getCHROM(vcf_obj), "_", getPOS(vcf_obj))[is_missing]
  return(ids)
}
...
# write a placeholder PDF 
write_no_data_pdf <- function(file, message = "No data available for the current filter selection.") {
...
coerce_and_validate_columns <- function(df, numeric_cols, character_cols, file_label) {
  bad_cols <- character(0)
  for (col in numeric_cols) {
    coerced <- suppressWarnings(as.numeric(df[[col]]))
    if (anyNA(coerced)) {
  ...
  validate(
    need(length(bad_cols) == 0,
         paste0("Invalid ", file_label, " file: column(s) ", paste(bad_cols, collapse = ", "),
                " must contain only numbers, with no missing values."))
  )
...
server <- function(input, output, session) {
  ...
  observeEvent(input$access_button, {
    if (input$data_choice == "Private data (login/registration required)") {
  ...
  output$upload_header <- renderUI({
    if (session_status$data_scope == "private") {
      h3(icon("upload"), "Processing sensitive data")
    } else {
      h3(icon("upload"), "Processing non-sensitive data")
    }
  })
  ...
  output$session_mode_display <- renderPrint({
    if (session_status$data_scope == "private") {
      cat(paste("PRIVATE/SENSITIVE DATA:", session_status$user))
    } else {
  ...
  vcfRInput <- reactive({
    ...
    read_and_validate <- function(path) {
      ...
      if (n_variants > MAX_VARIANTS)
        return(notify_err(paste0(
          "VCF file has too many variants (", n_variants, ") for interactive processing. ",
          "Please filter to ", MAX_VARIANTS, " variants or fewer (e.g. via bcftools region/MAF filtering) before uploading."
        )))
      if (n_samples > MAX_SAMPLES)
        return(notify_err(paste0(
          "VCF file has too many samples (", n_samples, ") for interactive processing. ",
          "Please use a cohort of ", MAX_SAMPLES, " samples or fewer."
        )))
    ...
    if (!is.null(input$file)) {
      ...
      file_size_mb <- round(file_info$size / 1024^2, 1)
  ...
  getSubInformationAsMatrix <- reactive({
    ...
    metafile <- tryCatch({
      ...
      if (!is.null(input$fileSubInfo)) {
        read.table(input$fileSubInfo$datapath, header = F, sep="\t", fill = T, quote = "", comment.char = "")
      } else if (isTRUE(input$meta1000G)) {
        read.table("https://raw.githubusercontent.com/TimHasenbein/Haplofun/main/01_data/050226_meta_subregions_adj.tsv", header = F, sep="\t", fill = T, quote = "", comment.char = "")
      } else if (isTRUE(input$metahinf)) {
        read.table("https://raw.githubusercontent.com/TimHasenbein/Haplofun/main/01_data/050226_meta_hinf_linreg_resistance.tsv", header = F, sep="\t", fill = T, quote = "", comment.char = "")
      }
    }, error = function(e) {
      validate(need(FALSE, paste("Could not read sample information file:", conditionMessage(e))))
    })
    validate(
      need(ncol(metafile) >= 2,
           paste("Invalid sample information file: expected at least 2 columns (sample ID, trait/group), but found",
                 ncol(metafile), "."))
    )
  ...
  observeEvent(input$applyFilters, {
    ...
    if (is.na(input$startPos) || is.na(input$endPos)) {
      showNotification("Start and End position must not be empty.", type = "error")
      return()
    }
    ...
    # MAF filter
    if (is.na(input$mafFilterInit)) {
      showNotification("MAF must not be empty.", type = "error")
      return()
    }
  ...
  vcfPreLDPruning_vcfBin <- reactive({
    vcf <- filteredVcfData_pos()
    withProgress(message = 'Data conversion...', value = 0, {
      incProgress(0.5)
      vcfData <- vcfR2DNAbin(vcf, extract.indels = TRUE)
  ...
  r2MatrixBeforePruning <- reactive({
    tryCatch({
      R.utils::withTimeout({
        ...
        snp_ids <- colnames(vcf)[ape::seg.sites(vcf)]
        rownames(r2_matrix) <- snp_ids
        colnames(r2_matrix) <- snp_ids
        return(r2_matrix)
      }, timeout = LD_SCAN_TIMEOUT_SECONDS, onTimeout = "error")
    }, error = function(e) {
  ...
  observeEvent(input$applyLdPruning, {
    ...
    if (!is.numeric(input$ldThreshold) || is.na(input$ldThreshold)) {
      showNotification("LD threshold (r²) must not be empty and must be a number.", type = "error")
      return()
    }
    # rm bootstrapping
    sampled_vcf_store(NULL)
    ...
    if (ncol(r2_matrix) <= 1) {
      showNotification("Only one or no segregating variants available for LD pruning. Adjust filters.", type = "warning")
      return()
    }
  ...
  output$ldHeatmapAfter <- renderPlot({
    ...
    r_matrix <- as.matrix(r_matrix)
    r2_matrix <- r_matrix^2
    snp_ids <- colnames(vcf.filtered)[ape::seg.sites(vcf.filtered)]
    rownames(r2_matrix) <- snp_ids
    colnames(r2_matrix) <- snp_ids
    upper_tri <- get_upper_tri(r2_matrix)
    melted_r2_matrix <- reshape2::melt(upper_tri, varnames = c("Var1", "Var2"), na.rm = TRUE)
  ...
  observeEvent(input$resetFilters, {
    ...
    new_max <- min(rows, MAX_HAPLOTYPES_FOR_NETWORK)
  ...
  observeEvent(input$applySnpsampling, {
    ...
    if (!is.numeric(sampling_value) || is.na(sampling_value)) {
      showNotification("Sample size must not be empty and must be a number.", type = "error")
      return()
    }
    if (sampling_value <= 1) {
  ...
  getAllinitHaplotypes <- reactive({
    tryCatch({
      ...
      }, timeout = COMPUTE_TIMEOUT_SECONDS, onTimeout = "error")
    }, error = function(e) {
      if (inherits(e, "TimeoutException")) {
        validate(need(FALSE,"High computational load detected. Please filter your data further."))
      } else {validate(need(FALSE, paste(e$message)))}
  ...
  calculateHaplonet <- reactive({
    ...
    if (num_haplotypes > MAX_HAPLOTYPES_FOR_NETWORK) {
      return(NULL) 
    }
    ...
    if (input$selectNetwork == "2") {
      haplonet <- tryCatch({
        ...
        if (inherits(e, "TimeoutException")) {
          validate(need(FALSE, "High computational load detected or too many haplotypes with missing data remaining. Please filter your data further."))
        } else {validate(need(FALSE, paste(e$message)))}
      })
    }
  ...
  observe({
    req(filteredVcfData())
    req(getHaplotypes())
    new_haplonet <- tryCatch(
      calculateHaplonet(),
      error = function(e) {
        if (!inherits(e, "shiny.silent.error")) {
          showNotification(paste("Network calculation failed:", conditionMessage(e)),
                           type = "error", duration = 10)
        }
        NULL
      }
    )
    haplonet(new_haplonet)
  })

  observe({
    net <- haplonet()
    req(net)
    if (is.null(attr(net, "alter.links"))) {
      updateSliderInput(session = session, inputId = "sliderThreshold", max = 0)
    } else {
      updateSliderInput(session = session, inputId = "sliderThreshold", max = max(attr(net, "alter.links")[, 3]))
    }
  })
  ...
  output$plotHaplotypeTable <- DT::renderDataTable({
    tryCatch({
      R.utils::withTimeout({
        DT::datatable(calculateHaplotypeTable(), options = list(scrollY = "350px",scrollX = TRUE))
      }, timeout = COMPUTE_TIMEOUT_SECONDS, onTimeout = "error")
    }, error = function(e) {
      if (inherits(e, "TimeoutException")) {
        validate(need(FALSE,"High computational load detected. Please filter your data further."))
  ...
  # haplotype sequences output
  output$plotSampleTable <- DT::renderDataTable({
    tryCatch({
      R.utils::withTimeout({
        DT::datatable(getSampleswithHaplo(), options = list(scrollY = "350px",scrollX = TRUE))
      }, timeout = COMPUTE_TIMEOUT_SECONDS, onTimeout = "error")
    }, error = function(e) {
      if (inherits(e, "TimeoutException")) {
        validate(need(FALSE,"High computational load detected. Please filter your data further."))
  ...
  haplonetPlotReactive <- reactive({
    ...
    if (subInputSelected() && input$checkboxMeta) {
      ...
      for (i in seq_len(nrow(status_info))) {
      ...
      gwas_sizes <- { haplo_freq <- haplo_counts[network_order]; haplo_freq[is.na(haplo_freq)] <- 0; as.numeric(haplo_freq) + 1 }
      plot(
        haplonetObj,
        pie = pie_matrix,
        bg = plot_colors,
      ...
      network_order <- labels(haplonetObj)
      haplo_counts <- table(status_info$haplotypes)
      qtl_sizes <- { haplo_freq <- haplo_counts[network_order]; haplo_freq[is.na(haplo_freq)] <- 0; as.numeric(haplo_freq) + 1 }
      plot(
        haplonetObj,
        pie = pie_matrix,
        bg = plot_colors,
  ...
  output$plotNetwork <- renderPlot(
    ...
    res = 96,
    {
      tryCatch({
        ...
        }, timeout = NETWORK_PLOT_TIMEOUT_SECONDS, onTimeout = "error")
    }, error = function(e) {
      if (inherits(e, "TimeoutException")) {
  ...
  map_sequence_to_top_haps <- reactive({
    ...
    num_top_haps <- min(input$sliderHaplotypes, length(hap_counts))
  ...
  finalPcaPlotReactive <- reactive({
    ...
    validate(
      need(ncol(pca$li) >= 2,
           "Not enough variation to compute 2 principal components. Please include more variants for PCA.")
    )
  ...
  plotAlleleHeatmapReactive <- reactive({
    ...
    if (total_heatmap_cells > MAX_HEATMAP_CELLS) {
      validate(
    ...
    calculateHaplotypeTable(), totalEntries()
  )
  ...
  output$plotAlleleHeatmap <- plotly::renderPlotly({
    ...
    if (totalEntries() > MAX_HEATMAP_CELLS) {
      validate(need(FALSE, "High computational load detected. Please filter your data further."))
    }
  ...
  GwasDataFile <- reactive({
    ...
    if (!is.null(input$GwasInfo)) {
      ...
      gwas_data <- coerce_and_validate_columns(
        gwas_data,
        numeric_cols = c("start", "end", "score", "pval", "effect_size"),
        character_cols = c("chromosome", "name", "strand", "trait", "risk_allele"),
        file_label = "GWAS"
      )
      return(gwas_data)
  ...
  output$gwasSnpTable <- DT::renderDataTable({
    ...
    DT::datatable(
      ...
      caption = "GWAS-SNPs that overlap haplogroups"
    )
  })
  ...
  eqtlDataFile <- reactive({
    ...
    if (!is.null(input$eQTLInfo)) {
      ...
      eqtl_data <- coerce_and_validate_columns(
        eqtl_data,
        numeric_cols = c("start", "end", "score", "pip", "afc"),
        character_cols = c("chromosome", "name", "strand", "tissue", "gene_name"),
        file_label = "QTL"
      )
      return(eqtl_data)
    }
  ...
  annoDataFile <- reactive({
    ...
    if (!is.null(input$annoInfo)) {
      ...
      anno_data <- coerce_and_validate_columns(
        anno_data,
        numeric_cols = c("start", "end", "score"),
        character_cols = c("chromosome", "name", "strand"),
        file_label = "annotation"
      )
      anno_data <- anno_data %>%
        dplyr::rename(
          chrom = `chromosome`
  ...
  getAnnoDataTable <- reactive({
    ...
    req(filteredVcfData())
    anno <- annoDataFile()
    if (isTRUE(input$annoStored1) || isTRUE(input$annoStored2)) {
      # these two files are unconverted UCSC/ENCODE cCRE BED exports (0-based half-open)
  ...
  output$buttonplotAnnoHeatmap <- downloadHandler(
    ...
    content = function(file) {
      ...
      plot_data <- calculateAnnotationHeatmapData()
      if (is.null(plot_data) || nrow(plot_data) == 0) {
        write_no_data_pdf(file)
        return()
      }
  ...
  output$annoPlot <- plotly::renderPlotly({
    ...
    if (is.null(plot_data) || nrow(plot_data) == 0) {
      return(
        plotly::ggplotly(ggplot() +
                           annotate("text", x = 0.5, y = 0.5, label = "No data to plot", size = 6) +
                           theme_void()
        ))
    }
  ...
  output$igv_viewer <- igvShiny::renderIgvShiny({
    igv_options <- igv_options_reactive()
    igvShiny::igvShiny(igv_options)
  })

  #output$igvUI_wrapper <- renderUI({
  #  igvShiny::igvShinyOutput("igv_viewer", height = "75vh")
  #})
  ...
  output$performanceWarning <- renderUI({
    ...
    is_large <- cell_count > PERFORMANCE_WARNING_CELLS
    if (is_large) {
  ...
  category_colors <- reactive({
    ...
    cols <- c("#246ca7ff","#fcb9cfff","#7bc3f9ff","#319533ff","#840e0eff",
              "#B74912FF", "#6DF2D0FF", "#1220B7FF", "#F26DC5FF", "#9C12B7FF",
              "#43B9E0FF", "#991E70FF", "#B7122DFF", "#471E99FF", "#5043E0FF",
              "#50E043FF", "#F2D06DFF", "#F2786DFF", "#66991EFF", "#1E9984FF",
              "#BA6DF2FF", "#8EF26DFF", "#12B712FF", "#E04391FF", "#E09E43FF",
              "#A9B712FF", "#997A1EFF", "#43E084FF", "#E04350FF", "#6D83F2FF",
              "#12B7B7FF", "#C6E043FF"
    )
  ...
  output$percentage <- renderUI({
    ...
    num_selected <- min(input$sliderHaplotypes, length(frequencyAllHaplotypes))
    frequencySelectedHaplotypes <-
      frequencyAllHaplotypes[1:num_selected]
    ...
    p("Comprises ",percentage,"% of the samples")
  })
